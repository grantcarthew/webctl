...
8. Schema preview. Add `--schema`. It requires an index; `network --schema` without one is an error directing the user to supply an entry. `network <n> --schema` parses entry n's response body as JSON and returns a key skeleton:
