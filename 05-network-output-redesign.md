...
- Seq-based `--range`: selecting entries by inclusive `seq` membership matching the displayed indices, replacing the current positional-slice semantics.
...
1. Detail dial. Add `--detail` with values `summary`, `standard`, `full`, defaulting to `standard`. It controls text rendering only and is ignored in JSON mode and by `--schema`.
   ...
   - failed entries: the FAILED token on the main line and its failure reason (the `error:` line) render at every level, because the reason is the point of a failed entry. The transport block still follows the standard rule and the request body follows the full rule, so a failed entry at summary is its main line plus the reason and nothing else.
...
- Follow the documentation conventions in AGENTS.md. Create docs/network.md (it does not exist yet; only serve.md and start.md have per-command docs) covering the new flags, the detail levels, the indexed output, drill-down, schema, and the `--find` workflow (find matches URL and body text to narrow the list; the matched body is seen by drilling into the entry, since the default level renders no bodies). Keep the in-CLI help (`networkCmd.Long`, its "Response formats" block, examples, and per-flag descriptions) in sync with docs/network.md rather than leaving it describing the old always-bodies output. No network agent-help topic exists, so none needs updating.
...
2. Restructure `format.Network` to honor the level: summary prints only the main line; standard adds the transport detail block; full adds bodies. Move body rendering behind the full level. Apply the same gate to the failed-request branch, but keep its `error:` reason line at every level so a failed entry at summary still names why it failed; only its request body drops to full. Change the subordinate indent to seven spaces.
3. Prefix each entry's main line with its zero-padded `seq` and no brackets. Confirm the value comes from the entry, not from its position in the slice. Redefine `--range START-END` in `applyNetworkLimiting` as inclusive `seq` membership (keep entries with `START <= seq <= END`) rather than a positional `entries[start:end]` slice, and update the flag help accordingly; leave `--head`/`--tail` as counts.
...
8. Create docs/network.md documenting the network flags and output, and rewrite the in-CLI help to match: `networkCmd.Long`, its "Response formats" block, the examples, and the per-flag descriptions must describe the new default indexed view, the detail levels, drill-down, schema, and seq-based `--range`, so `webctl network --help` and docs/network.md agree. No network-specific agent-help topic exists today, so there is none to update.
9. Extend internal/cli/network_test.go and format tests to cover the detail levels, indexed display, seq-based `--range` (a sparse held set where the endpoints are absent but interior seqs match, and an interval that holds nothing), drill-down hit and miss, the max-body-size regimes, schema on JSON and non-JSON bodies, and schema without an index. The miss coverage must include a sparse held set (a seq between the lowest and highest that is absent) to prove the lookup tests membership rather than range, and the empty-buffer message.
10. Update the bash CLI suite (scripts/test/cli/test-observation.sh) for the new default, since the changed command surface is exercised by `./test-runner ci`. The default view now renders no bodies, so any assertion that a body payload appears in default `network` output must move to the two-step flow: the entry is listed by its main line, and the body is surfaced by drill-down (`network <n>`) or `--detail full`. Concretely, the `--find` request-body test (currently asserting the `obo-reqbody` payload appears in default `network --find` stdout) must assert the entry is listed and then drill in to see the payload, rather than expecting the body in the list.
...
`--find` keeps its existing reach — it matches URL and body text — but the default level no longer renders bodies, so a match narrows the list without showing the matched text. That is the intended two-step flow: `--find` surfaces the entries worth looking at, then drill-down (`network <n>`) shows the payload. Do not special-case `--find` to promote matched bodies back into the list; that would rebuild the wall of text the redesign removes.
...
- `network --range 318-425` returns every entry whose `seq` is in `[318, 425]` inclusive, matching the displayed indices, and returns an empty list when the active session holds no seq in that interval; `--head`/`--tail` remain entry counts.
