...
func TestFind(t *testing.T) {
	...
	var buf bytes.Buffer
	opts := OutputOptions{UseColor: false}
	err := Find(&buf, data, opts)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	output := buf.String()
	if !strings.Contains(output, "Match 1 of 2") {
		t.Error("output should contain match header")
	}
	if !strings.Contains(output, "> this is a test match") {
		t.Error("output should contain match line with > prefix")
	}
	if !strings.Contains(output, "before text") {
		t.Error("output should contain before context")
	}
	if !strings.Contains(output, "---") {
		t.Error("output should contain separator between matches")
	}
}
