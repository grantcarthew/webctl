...
- No command in the repository currently uses cobra `Aliases`; short forms today come only from `Execute`'s unique-prefix expansion. `md` is not a prefix of `markdown`, so it must be a real cobra alias — the first in the repo. Command registration is split across two registries: the CLI layer auto-discovers commands via `rootCmd.Commands()`, but the REPL (`internal/daemon/repl.go`) keeps its own hardcoded `webctlCommands` list for prefix expansion plus a `printHelp` listing, both of which must be updated by hand.
...
7. A shared save helper that every save command uses — `html save`, `css save`, `console save`, `network save`, and `markdown save` — with five per-command variation points: timer label, content producer, temp dir, file extension, and identifier source. After this change, no per-command save function contains duplicated path-resolution, filename-generation, or file-writing logic. The content producer is `func(*cobra.Command) (string, error)` and returns the final file payload — the exact bytes the helper writes to disk — not the displayed output. For the content commands (`html`, `css`, `markdown`) that payload is the content string itself. For the buffer commands (`console`, `network`) the producer marshals the existing JSON envelope (`{"ok":true,"logs"/"entries":[...],"count":N}`), with `network` applying `max-body-size` body truncation before marshalling, so the saved-file format is unchanged and the existing console/network save tests still pass. File content is decoupled from the helper's always-uniform success output (`{"ok":true,"path":...}` in JSON mode, `format.FilePath` in text mode). The helper generates filenames as `YY-MM-DD-HHMMSS-mmm[-identifier].{ext}`: a millisecond-precision timestamp (so two saves within the same second no longer overwrite each other) followed by an optional identifier. The identifier source is one of two kinds: the content commands (`html`, `css`, `markdown`) resolve it as the sanitized selector if given, otherwise the normalized page title, otherwise omitted — including when the `status` lookup fails, in which case the save still succeeds; the buffer commands (`console`, `network`) supply a fixed word (`console`, `network`). This replaces the previous per-command `page`/`untitled` fallback words and the `css` abort-on-status-error behavior.
...
5. Add the `markdown save` subcommand wired to the shared save helper with the markdown content producer (the markdown getter doubles as the producer, since its content string is the file payload), `/tmp/webctl-markdown`, and `.md`.
6. Register the command in `init()` and add it to `commandGroups` (`internal/cli/root.go`) in the same group as `html` and `css`. The REPL keeps a separate hardcoded command registry: add `markdown` to `webctlCommands` (`internal/daemon/repl.go`) so REPL prefix expansion resolves `mark`/`m`, and include it in the `printHelp` command listing. The `md` alias needs no list entry — cobra `Find` resolves the alias regardless.
