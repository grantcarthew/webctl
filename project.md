...
7. A persistent profile must not be left unusable by a stale Chrome singleton lock after an unclean browser exit. First determine whether Chrome already recovers a stale lock on relaunch against the default profile. Add explicit lock clearing only for cases Chrome does not handle on its own, and only when the lock is confirmed stale: the recorded PID is not a live process and the host matches. Never remove a lock that may belong to a running browser. If explicit clearing is added, it belongs on the `webctl start` path, immediately before relaunch, where the resolved profile path is in hand for every mode. Do not place the clearing in `webctl stop --force` or in daemon shutdown: a stale lock is created precisely by a process that died without cleaning up (crash, SIGKILL, force), so any cleanup on the death path is unreliable for the very case it targets, and `stop --force` has no record of which profile the dead daemon used. The next `webctl start` is the only point guaranteed to run and to know the path, so it is the single source of truth for stale-lock recovery.

8. A persistent profile must not surface Chrome's crash or session restore UI on the next `webctl start` after an unclean exit. `Browser.Close()` terminates the browser with SIGTERM and `webctl stop --force` SIGKILLs orphans, neither of which reliably records a clean exit in the profile. With a throwaway profile this was invisible because the directory was deleted; with a reused persistent profile Chrome can come up showing the "Restore pages?" crash-restore bubble and reopen the previous session's tabs. The restore bubble overlays the active page and breaks screenshots and click coordinates, and resurrected tabs change which page target the daemon treats as active. Determine empirically whether this occurs on relaunch of the dedicated default profile (alongside the singleton-lock check). If it does, suppress it with the `--hide-crash-restore-bubble` launch flag, and evaluate `--disable-session-crash-bubble`. Emit these flags in the same `buildArgs` branch that already emits `--user-data-dir` — that is, whenever `UserDataDir` resolves to a concrete path. By the time `buildArgs` runs, `spawnProcess` has assigned the temp directory into `UserDataDir`, so the persistent default, `--user-data-dir`, and temp modes are all indistinguishable concrete paths; gating on the `--user-data-dir` emission covers the persistent default and `--user-data-dir` modes and also reaches temp, where the flags are inert because a freshly created profile has no prior session to restore. The browser layer does not need to learn which mode produced the path. The `--system-profile` mode is excluded automatically: it carries the `UserDataDirDefault` sentinel and emits no `--user-data-dir` flag, so it falls outside this branch — which is correct, because it borrows the user's real profile and webctl must not alter its restore behaviour. Do not mutate the profile's `Preferences` file to fake a clean exit; the format is version-fragile.

9. `docs/start.md` documents the profile modes and the default location. `webctl start --help` describes the new flags. The `--system-profile` documentation warns that it requires no other Chrome instance to be running on the default profile: a running instance was not started with remote debugging, the new launch forwards to it and exits, and webctl cannot attach. The persistent default avoids this by using its own dedicated directory.

10. `--system-profile` fails fast instead of hanging. When that mode is selected and the launched process exits within a short window without the CDP endpoint coming up, webctl reports a targeted error explaining the likely cause (an existing Chrome instance holds the default profile) rather than waiting out the full CDP start timeout.
...
5. Verify relaunch behavior on the default profile before building anything: force-kill the browser, then run `webctl start` against the default profile and observe two things at once — whether Chrome recovers a stale singleton lock on its own, and whether it shows the crash/session-restore bubble or reopens prior tabs. For the lock, add clearing only if Chrome does not recover, gated on a confirmed-stale lock (recorded PID not alive, host matches), and place it on the `webctl start` path immediately before relaunch — not in `stop --force` or daemon shutdown, since death-path cleanup is unreliable for a lock created by an uncatchable termination; do not delete a lock that may be live. For crash-restore, if the bubble or tab restoration appears, add `--hide-crash-restore-bubble` (and evaluate `--disable-session-crash-bubble`) inside the existing `buildArgs` branch that emits `--user-data-dir`, so they apply to every concrete-path mode (persistent default, `--user-data-dir`, and harmlessly temp) and are skipped for `--system-profile`, which emits no `--user-data-dir` flag. Do not thread mode information into the browser layer to distinguish temp from the persistent default; the flags are inert on a fresh temp profile. Do not edit the profile `Preferences` file to fake a clean exit.
...
- After a browser is force-killed, a subsequent `webctl start` against the default profile launches successfully without manual lock removal, and comes up clean: no crash/session-restore bubble overlaying the active page and no resurrected tabs from the prior session.
