...
14. Unbounded CDP read limit is unsafe for remote attach (gap) — Resolution: make the read limit trust-dependent.

    `internal/cdp/client.go` `Dial` sets `conn.SetReadLimit(-1)` (unbounded). This is correct for a launched, loopback Chrome: it is a trusted peer that legitimately sends large messages (full response bodies, DOM snapshots, screenshots) whose size is bounded by real page data, and a finite limit is fatal in coder/websocket — exceeding it tears down the CDP connection and forces the daemon to kill the browser. The unbounded read is what lets the daemon retain large bodies instead of crashing.
    A remote, non-loopback CDP endpoint (Issue 6) inverts the trust assumption: CDP is unauthenticated, so a hostile or buggy remote endpoint can stream an arbitrarily large frame and exhaust daemon memory. An unbounded read is then a denial-of-service vector on the same surface Issue 6 already flags as a footgun.
    Resolution: the attach path must make the read limit trust-dependent rather than always `-1`. Loopback endpoints (launched or attached) keep the unbounded limit. Non-loopback attach sets a finite limit before `cdp.Dial`; for an untrusted peer the fatal cliff is the correct tradeoff — protecting the daemon outweighs retaining one oversized frame. Plumb the chosen limit through `cdp.Dial` (e.g. an explicit limit argument or option) so the caller selects it by host trust; the daemon resolves trust from `cfg.AttachHost` being non-loopback. Pairs with the CDP-no-auth warning already required for non-loopback hosts.
