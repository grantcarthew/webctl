...
- internal/cli/network.go
  - Filters (`--method`, `--status`, `--type`, `--url`, `--mime`, `--failed`, head/tail/range), `--find` substring search (currently searches `entry.URL` and `entry.Body`), `--max-body-size` truncation (default 102400 bytes) applied to `Body` in both JSON and raw paths, and `webctl network save`.
...
6. Surface in text output. The text formatter labels the request body `request:` and the response body `response:`, request line first, each indented two spaces, with no arrow or other glyph prefix. Print each line whenever its body is present and non-empty, gating on presence alone for both. Drop the current `Method != "GET"` gate on the response body: it is a vestige of the original request-body intent (GETs have no request body) and has no meaning for a response body, where a GET's payload is as worth showing as any other method's. Request and response thus display symmetrically. Text output bounds each printed body by `--max-body-size` using the same shared truncation helper as the JSON and save paths (Implementation Plan step 5), applied to the entries before rendering; the formatter prints already-bounded text and does not reintroduce a method exception. Binary response bodies remain filed rather than printed, so their `Body` is empty and nothing prints for them. A single-line body follows the label on the same line after one space. A multi-line body prints the bare label line, then each body line indented four spaces. The exact form:
...
3. Capture omitted bodies. When `hasPostData` is true but `postData` is absent, mark the just-pushed entry as awaiting its body, then fetch the body via `Network.getRequestPostData` using the event's session and request id, off the read loop, following the `handleLoadingFinished` goroutine pattern. Store the awaiting marker as a per-entry field on `NetworkEntry` that is excluded from JSON serialization (a `json:"-"` tag), not as a separate side structure. The marker must travel with the entry because the buffer stores entries by value and `RingBuffer.RemoveIf` compacts (copies) them, so there is no stable external identity to key a side map on; a map keyed by `requestId` also cannot distinguish redirect hops that share the id. Excluding the field from JSON means a `webctl network` call that races an in-flight fetch cannot serialize a transient pending flag into output. When the body arrives, update the newest entry that carries the awaiting marker for this `requestId`, not merely the newest entry sharing the `requestId`. Matching on the marker rather than on an empty `requestBody` is what prevents theft: a non-body redirect hop (for example a POST that 303-redirects to a GET) shares the `requestId`, is the newer entry, and has an equally empty `requestBody`, so an emptiness test would misroute the body onto it. In the rare case where two hops share the `requestId` and both carry the marker (a body-preserving 307/308 redirect), both fetches return the identical body, so the newest-first match needs no oldest/newest tie-break. Treat the "no data was sent" error as a non-error (nothing to store; clear the awaiting marker).
...
5. Truncate at the CLI. Introduce a single truncation helper in internal/cli/network.go that cuts a body to the `--max-body-size` byte budget on a UTF-8 rune boundary (the longest valid prefix not exceeding the budget, so a multibyte rune is never split) and reports whether it truncated. Apply this one helper to both the request and response bodies in all three output paths — JSON, save, and text — truncating the entries before they are rendered, each setting its own truncation flag (`requestBodyTruncated`, `bodyTruncated`). The JSON and save paths surface the flags; the text path simply prints the bounded body. This replaces the current inline byte-slice on the response body, and brings truncation to the text path, which today prints bodies in full; the logic lives in one place rather than scattered copies.

6. Search the request body. Extend the `--find` predicate in internal/cli/network.go to also match the request body.

7. Render in text. In internal/cli/format/text.go, print the request body for requests that have one, labeled distinctly from the response body, and correct the existing mislabeled block. The entries reaching the formatter are already truncated by step 5, so the formatter renders their bodies verbatim; it does not call the truncation helper itself.
...
- Truncation is settled: both bodies share the single `--max-body-size` threshold (no second flag), and the request body reports its own truncation via `requestBodyTruncated`. Route both bodies through the one rune-aware helper from Implementation Plan step 5 in the JSON, save, and text paths, rather than copying the byte-slice per body and path.
