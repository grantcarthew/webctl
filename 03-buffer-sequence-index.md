...
2. Resolve how a generic buffer stamps a field on an arbitrary element type. The buffer is `RingBuffer[T any]`; setting a field on `T` requires either a type constraint with a pointer-receiver setter that both entry types implement, or a stamping function supplied at construction. Pick whichever keeps the buffer fully general; do not branch on the concrete type.
3. Add the `seq` field to `ConsoleEntry` and `NetworkEntry` and wire each type into the chosen stamping mechanism.
